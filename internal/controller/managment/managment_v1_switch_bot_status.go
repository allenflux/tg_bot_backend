...
func (c *ControllerV1) SwitchBotStatus(ctx context.Context, req *v1.SwitchBotStatusReq) (res *v1.SwitchBotStatusRes, err error) {
	...
	status := consts.BotStatus

	switch bots[0].Status {
	case consts.BotStatusAvailable:
		status = consts.BotStatusUnAvailable
	case consts.BotStatusUnAvailable:
